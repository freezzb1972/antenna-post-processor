...
def pattern_radius(values_db: np.ndarray, dyn: float = DEFAULT_DYN_DB) -> np.ndarray:
    """标准半径归一化: dB 值 → [0,1] 半径。

    峰值 → 1 (最外), 峰值−dyn 及以下 → 0 (收缩到中心)。
    这样峰值鼓出、零点收缩, 呈现真实方向性波瓣。
